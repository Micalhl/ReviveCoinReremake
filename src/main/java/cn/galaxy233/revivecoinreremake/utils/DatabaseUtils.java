...
public class DatabaseUtils {
    ...
    private static final PPlugin plugin = ReviveCoinReremake.getInstance();
            ...
            try (PreparedStatement statement = connection.prepareStatement("SELECT `coins` FROM `revivecoin` WHERE `uuid` = ?")) {
                ...
                if (!resultSet.next()) {
                    try (PreparedStatement insertS = connection.prepareStatement(
                        ...
                        String message = plugin.lang.get(plugin.localeKey, I18n.Type.INFO, "Lang", "new-player-gived-coins").replace("{coin}", String.valueOf(defaultCoins));
